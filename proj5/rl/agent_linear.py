...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if np.random.random() <= epsilon:
        c_idx = np.random.choice(len(q))
    else:
        c_idx = np.argmax(q)
    action_index, object_index = index2tuple(c_idx)
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    """
    # TODO Your code here
    q = theta @ current_state_vector
    q_next = theta @ next_state_vector
    c_idx = tuple2index(action_index, object_index)
    phi = np.zeros([len(q), len(current_state_vector)])
    phi[c_idx] = current_state_vector

    if terminal:
        theta += ALPHA * (reward - q[c_idx]) * phi
    else:
        theta += ALPHA * (reward + GAMMA * np.max(q_next) - q[c_idx]) * phi
# pragma: coderesponse end
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        action, object = epsilon_greedy(current_state_vector, theta, epsilon)
        ...
        if for_training:
            # update Q-function.
            # TODO Your code here
            linear_q_learning(theta, current_state_vector, action, object, reward, next_state_vector, terminal)
            pass
